...
for(let i=0; i<NUMBER_OF_FIGHTS; i++) {
	for(let stat in stats) {
		const d1 = f.generateFocusedDragon(100, stat, 70)
		for(let enemy in stats) {
			const d2 = f.generateFocusedDragon(100, enemy, 70)
...
for(let i=1; i<=NUMBER_OF_FIGHTS; i++) {
	...
	const d2 = { con: 10, int: 10, agl: 10, str: 10, wlp: i, lck: 1, level: i+40, terrain: 0 }
